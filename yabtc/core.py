...
def hash160(data: bytearray) -> bytearray:
    return bytearray(yabtc.ripemd160.ripemd160(hashlib.sha256(data).digest()).digest())
...
def hashtag(name: str, data: bytearray) -> bytearray:
    return yabtc.schnorr.hash(name, data)
...
class PriKey:
    ...
    def pubkey(self):
        pubkey = yabtc.secp256k1.G * yabtc.secp256k1.Fr(self.n)
        return PubKey(pubkey.x.x, pubkey.y.x)

    def sign_ecdsa(self, data: bytearray) -> typing.Tuple[yabtc.secp256k1.Fr, yabtc.secp256k1.Fr, int]:
        assert len(data) == 32
        m = yabtc.secp256k1.Fr(int.from_bytes(data))
        for _ in itertools.repeat(0):
            r, s, v = yabtc.ecdsa.sign(yabtc.secp256k1.Fr(self.n), m)
    ...
    def sign_schnorr(self, data: bytearray) -> bytearray:
        assert len(data) == 32
        m = yabtc.secp256k1.Fr(int.from_bytes(data))
        r, s = yabtc.schnorr.sign(yabtc.secp256k1.Fr(self.n), m)
        return bytearray(r.x.x.to_bytes(32) + s.x.to_bytes(32))

    def wif(self) -> str:
        # See https://en.bitcoin.it/wiki/Wallet_import_format
        data = bytearray()
        data.append(yabtc.config.current.prefix.wif)
        data.extend(self.n.to_bytes(32))
        data.append(0x01)
        checksum = hash256(data)[:4]
        data.extend(checksum)
        return yabtc.base58.encode(data)

    @classmethod
    def wif_decode(cls, data: str) -> typing.Self:
        data = yabtc.base58.decode(data)
        assert data[0] == yabtc.config.current.prefix.wif
...
class PubKey:
    def __init__(self, x: int, y: int) -> None:
        ...
        _ = yabtc.secp256k1.Pt(yabtc.secp256k1.Fq(x), yabtc.secp256k1.Fq(y))
        self.x = x
        self.y = y
    ...
    def pt(self) -> yabtc.secp256k1.Pt:
        return yabtc.secp256k1.Pt(yabtc.secp256k1.Fq(self.x), yabtc.secp256k1.Fq(self.y))

    @classmethod
    def pt_decode(cls, data: yabtc.secp256k1.Pt) -> typing.Self:
    ...
    def sec_decode(cls, data: bytearray) -> typing.Self:
        ...
        if p == 0x04:
            ...
            y_x_y = x * x * x + yabtc.secp256k1.A.x * x + yabtc.secp256k1.B.x
            y = pow(y_x_y, (yabtc.secp256k1.P + 1) // 4, yabtc.secp256k1.P)
            if y & 1 != p - 2:
                y = -y % yabtc.secp256k1.P
        return PubKey(x, y)
...
def address_p2pkh(pubkey: PubKey) -> str:
    ...
    data = bytearray([yabtc.config.current.prefix.p2pkh]) + pubkey_hash
    chk4 = hash256(data)[:4]
    return yabtc.base58.encode(data + chk4)
...
def address_p2sh(redeem: bytearray) -> str:
    ...
    data = bytearray([yabtc.config.current.prefix.p2sh]) + redeem_hash
    chk4 = hash256(data)[:4]
    return yabtc.base58.encode(data + chk4)
...
def address_p2sh_p2ms(n: int, pubkey: typing.List[PubKey]) -> str:
    ...
    for e in pubkey:
        redeem_script.append(yabtc.opcode.op_pushdata(e.sec()))
    redeem_script.append(yabtc.opcode.op_n(len(pubkey)))
    redeem_script.append(yabtc.opcode.op_checkmultisig)
...
def address_p2sh_p2wpkh(pubkey: PubKey) -> str:
    ...
    pubkey_hash = hash160(pubkey.sec())
    redeem_script = script([yabtc.opcode.op_0, yabtc.opcode.op_pushdata(pubkey_hash)])
    return address_p2sh(redeem_script)
...
def address_p2wpkh(pubkey: PubKey) -> str:
    ...
    pubkey_hash = hash160(pubkey.sec())
    return yabtc.bech32.encode(yabtc.config.current.prefix.bech32, 0, pubkey_hash)
...
def address_p2tr(pubkey: PubKey, root: bytearray) -> str:
    ...
    adjust_pubkey = yabtc.secp256k1.G * adjust_prikey
    output_pubkey = origin_pubkey + adjust_pubkey
    return yabtc.bech32.encode(yabtc.config.current.prefix.bech32, 1, bytearray(output_pubkey.x.x.to_bytes(32)))
...
class OutPoint:
    ...
    def load(self):
        rpcret = yabtc.rpc.get_tx_out(self.txid[::-1].hex(), self.vout)
        script_pubkey = bytearray.fromhex(rpcret['scriptPubKey']['hex'])
        amount = rpcret['value'] * yabtc.denomination.bitcoin
...
def script_pubkey_p2pkh(addr: str) -> bytearray:
    data = yabtc.base58.decode(addr)
    assert data[0] == yabtc.config.current.prefix.p2pkh
    hash = data[0x01:0x15]
    assert yabtc.core.hash256(data[0x00:0x15])[:4] == data[0x15:0x19]
    return script([
        yabtc.opcode.op_dup,
        yabtc.opcode.op_hash160,
        yabtc.opcode.op_pushdata(hash),
...
def script_pubkey_p2sh(addr: str) -> bytearray:
    data = yabtc.base58.decode(addr)
    assert data[0] == yabtc.config.current.prefix.p2sh
    hash = data[0x01:0x15]
    assert yabtc.core.hash256(data[0x00:0x15])[:4] == data[0x15:0x19]
    return script([
        yabtc.opcode.op_hash160,
...
def script_pubkey_p2tr(addr: str) -> bytearray:
    pubx = yabtc.bech32.decode(yabtc.config.current.prefix.bech32, 1, addr)
    return script([
        yabtc.opcode.op_1,
        yabtc.opcode.op_pushdata(pubx),
    ])


def script_pubkey(addr: str) -> bytearray:
    if addr.startswith(yabtc.config.current.prefix.bech32):
        if addr[len(yabtc.config.current.prefix.bech32) + 1] == 'q':
            return script_pubkey_p2wpkh(addr)
        if addr[len(yabtc.config.current.prefix.bech32) + 1] == 'p':
            return script_pubkey_p2tr(addr)
    if yabtc.base58.decode(addr)[0] == yabtc.config.current.prefix.p2pkh:
        return script_pubkey_p2pkh(addr)
    if yabtc.base58.decode(addr)[0] == yabtc.config.current.prefix.p2sh:
...
def der_encode(r: yabtc.secp256k1.Fr, s: yabtc.secp256k1.Fr) -> bytearray:
...
def der_decode(sign: bytearray) -> typing.Tuple[yabtc.secp256k1.Fr, yabtc.secp256k1.Fr]:
    assert sign[0] == 0x30
    assert sign[1] == len(sign) - 2
    assert sign[2] == 0x02
    rlen = sign[3]
    r = yabtc.secp256k1.Fr(int.from_bytes(sign[4:4+rlen]))
    f = 4 + rlen
    assert sign[f] == 0x02
    slen = sign[f+1]
    f = f + 2
    s = yabtc.secp256k1.Fr(int.from_bytes(sign[f:f+slen]))
    return r, s
...
class Message:
    ...
    def hash(self) -> bytearray:
        ...
        b.extend(yabtc.core.compact_size_encode(24))
        b.extend(bytearray('Bitcoin Signed Message:\n'.encode()))
        b.extend(yabtc.core.compact_size_encode(len(self.data)))
        b.extend(bytearray(self.data.encode()))
        return yabtc.core.hash256(b)
    ...
    def pubkey(self, sig: str) -> PubKey:
        m = yabtc.secp256k1.Fr(int.from_bytes(self.hash()))
        sig = base64.b64decode(sig)
        assert sig[0] >= 27
        v = (sig[0] - 27) & 3
        r = yabtc.secp256k1.Fr(int.from_bytes(sig[0x01:0x21]))
        s = yabtc.secp256k1.Fr(int.from_bytes(sig[0x21:0x41]))
        return PubKey.pt_decode(yabtc.ecdsa.pubkey(m, r, s, v))
