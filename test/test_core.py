...
def test_address_p2pkh():
    ...
    addr = plbtc.core.address_p2pkh(pubkey)
...
def test_address_p2sh():
    ...
    redeem = plbtc.core.script([
        plbtc.opcode.op_1,
        plbtc.opcode.op_pushdata(pubkey),
        plbtc.opcode.op_1,
        plbtc.opcode.op_checkmultisig,
    ])
    addr = plbtc.core.address_p2sh(redeem)
    assert addr == '3P14159f73E4gFr7JterCCQh9QjiTjiZrG'


def test_address_p2sh_p2ms():
    plbtc.config.current = plbtc.config.develop
    keys = [plbtc.core.PubKey.sec_decode(bytearray.fromhex(e)) for e in [
    ...
    addr = plbtc.core.address_p2sh_p2ms(2, keys)
...
def test_address_p2sh_p2wpkh():
    ...
    prikey = plbtc.core.PriKey(1)
    pubkey = prikey.pubkey()
    addr = plbtc.core.address_p2sh_p2wpkh(pubkey)
...
def test_address_p2tr():
    ...
    prikey = plbtc.core.PriKey(1)
    pubkey = prikey.pubkey()
    addr = plbtc.core.address_p2tr(pubkey, bytearray())
...
def test_compact_size():
    for n, b in [
        ...
        assert plbtc.core.compact_size_encode(n) == b
        assert plbtc.core.compact_size_decode(b) == n


def test_der():
    for _ in range(256):
        r0 = plbtc.secp256k1.Fr(random.randint(0, plbtc.secp256k1.N - 1))
        s0 = plbtc.secp256k1.Fr(random.randint(0, plbtc.secp256k1.N - 1))
        r1, s1 = plbtc.core.der_decode(plbtc.core.der_encode(r0, s0))
        assert r0 == r1
        assert s0 == s1


def test_difficulty_target():
    assert plbtc.core.difficulty_target(
...
def test_hash160():
    hash = plbtc.core.hash160(bytearray([0, 1, 2, 3]))
    assert hash.hex() == '3c3fa3d4adcaf8f52d5b1843975e122548269937'


def test_message():
    for _ in range(4):
        prikey = plbtc.core.PriKey(random.randint(0, plbtc.secp256k1.N))
        pubkey = prikey.pubkey()
        msg = plbtc.core.Message(''.join(random.choice(string.ascii_letters) for _ in range(random.randint(0, 1024))))
        sig = msg.sign(prikey)
...
def test_prikey_wif():
    ...
    assert prikey == plbtc.core.PriKey.wif_decode(prikey.wif())
    plbtc.config.current = plbtc.config.testnet
...
def test_pubkey_sec_read():
    pubkey = plbtc.core.PubKey.sec_decode(bytes.fromhex(''.join([
        '04',
...
def test_transaction():
    ...
    tx = plbtc.core.Transaction.serialize_decode(data)
...
def test_witness():
    for _ in range(256):
        ...
        assert plbtc.core.witness_decode(plbtc.core.witness_encode(wits)) == wits
