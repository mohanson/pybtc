...
def hash160(data: bytearray) -> bytearray:
    return bytearray(plbtc.ripemd160.ripemd160(hashlib.sha256(data).digest()).digest())
...
def hashtag(name: str, data: bytearray) -> bytearray:
    return plbtc.schnorr.hash(name, data)
...
class PriKey:
    ...
    def pubkey(self):
        pubkey = plbtc.secp256k1.G * plbtc.secp256k1.Fr(self.n)
        return PubKey(pubkey.x.x, pubkey.y.x)

    def sign_ecdsa(self, data: bytearray) -> typing.Tuple[plbtc.secp256k1.Fr, plbtc.secp256k1.Fr, int]:
        assert len(data) == 32
        m = plbtc.secp256k1.Fr(int.from_bytes(data))
        for _ in itertools.repeat(0):
            r, s, v = plbtc.ecdsa.sign(plbtc.secp256k1.Fr(self.n), m)
    ...
    def sign_schnorr(self, data: bytearray) -> bytearray:
        assert len(data) == 32
        m = plbtc.secp256k1.Fr(int.from_bytes(data))
        r, s = plbtc.schnorr.sign(plbtc.secp256k1.Fr(self.n), m)
        return bytearray(r.x.x.to_bytes(32) + s.x.to_bytes(32))

    def wif(self) -> str:
        # See https://en.bitcoin.it/wiki/Wallet_import_format
        data = bytearray()
        data.append(plbtc.config.current.prefix.wif)
        data.extend(self.n.to_bytes(32))
        data.append(0x01)
        checksum = hash256(data)[:4]
        data.extend(checksum)
        return plbtc.base58.encode(data)

    @classmethod
    def wif_decode(cls, data: str) -> typing.Self:
        data = plbtc.base58.decode(data)
        assert data[0] == plbtc.config.current.prefix.wif
...
class PubKey:
    def __init__(self, x: int, y: int) -> None:
        ...
        _ = plbtc.secp256k1.Pt(plbtc.secp256k1.Fq(x), plbtc.secp256k1.Fq(y))
        self.x = x
        self.y = y
    ...
    def pt(self) -> plbtc.secp256k1.Pt:
        return plbtc.secp256k1.Pt(plbtc.secp256k1.Fq(self.x), plbtc.secp256k1.Fq(self.y))

    @classmethod
    def pt_decode(cls, data: plbtc.secp256k1.Pt) -> typing.Self:
    ...
    def sec_decode(cls, data: bytearray) -> typing.Self:
        ...
        if p == 0x04:
            ...
            y_x_y = x * x * x + plbtc.secp256k1.A.x * x + plbtc.secp256k1.B.x
            y = pow(y_x_y, (plbtc.secp256k1.P + 1) // 4, plbtc.secp256k1.P)
            if y & 1 != p - 2:
                y = -y % plbtc.secp256k1.P
        return PubKey(x, y)
...
def address_p2pkh(pubkey: PubKey) -> str:
    ...
    data = bytearray([plbtc.config.current.prefix.p2pkh]) + pubkey_hash
    chk4 = hash256(data)[:4]
    return plbtc.base58.encode(data + chk4)
...
def address_p2sh(redeem: bytearray) -> str:
    ...
    data = bytearray([plbtc.config.current.prefix.p2sh]) + redeem_hash
    chk4 = hash256(data)[:4]
    return plbtc.base58.encode(data + chk4)
...
def address_p2sh_p2ms(n: int, pubkey: typing.List[PubKey]) -> str:
    redeem_script = []
    redeem_script.append(plbtc.opcode.op_n(n))
    for e in pubkey:
        redeem_script.append(plbtc.opcode.op_pushdata(e.sec()))
    redeem_script.append(plbtc.opcode.op_n(len(pubkey)))
    redeem_script.append(plbtc.opcode.op_checkmultisig)
...
def address_p2sh_p2wpkh(pubkey: PubKey) -> str:
    ...
    pubkey_hash = hash160(pubkey.sec())
    redeem_script = script([plbtc.opcode.op_0, plbtc.opcode.op_pushdata(pubkey_hash)])
    return address_p2sh(redeem_script)
...
def address_p2wpkh(pubkey: PubKey) -> str:
    ...
    pubkey_hash = hash160(pubkey.sec())
    return plbtc.bech32.encode(plbtc.config.current.prefix.bech32, 0, pubkey_hash)
...
def address_p2tr(pubkey: PubKey, root: bytearray) -> str:
    ...
    adjust_pubkey = plbtc.secp256k1.G * adjust_prikey
    output_pubkey = origin_pubkey + adjust_pubkey
    return plbtc.bech32.encode(plbtc.config.current.prefix.bech32, 1, bytearray(output_pubkey.x.x.to_bytes(32)))
...
class OutPoint:
    ...
    def load(self):
        rpcret = plbtc.rpc.get_tx_out(self.txid[::-1].hex(), self.vout)
        script_pubkey = bytearray.fromhex(rpcret['scriptPubKey']['hex'])
        amount = rpcret['value'] * plbtc.denomination.bitcoin
...
def script_pubkey_p2pkh(addr: str) -> bytearray:
    data = plbtc.base58.decode(addr)
    assert data[0] == plbtc.config.current.prefix.p2pkh
    hash = data[0x01:0x15]
    assert plbtc.core.hash256(data[0x00:0x15])[:4] == data[0x15:0x19]
    return script([
        plbtc.opcode.op_dup,
        plbtc.opcode.op_hash160,
        plbtc.opcode.op_pushdata(hash),
...
def script_pubkey_p2sh(addr: str) -> bytearray:
    data = plbtc.base58.decode(addr)
    assert data[0] == plbtc.config.current.prefix.p2sh
    hash = data[0x01:0x15]
    assert plbtc.core.hash256(data[0x00:0x15])[:4] == data[0x15:0x19]
    return script([
        plbtc.opcode.op_hash160,
...
def script_pubkey_p2wpkh(addr: str) -> bytearray:
    hash = plbtc.bech32.decode(plbtc.config.current.prefix.bech32, 0, addr)
    return script([
        plbtc.opcode.op_0,
        plbtc.opcode.op_pushdata(hash),
    ])
...
def script_pubkey_p2tr(addr: str) -> bytearray:
    pubx = plbtc.bech32.decode(plbtc.config.current.prefix.bech32, 1, addr)
    return script([
        plbtc.opcode.op_1,
        plbtc.opcode.op_pushdata(pubx),
    ])


def script_pubkey(addr: str) -> bytearray:
    if addr.startswith(plbtc.config.current.prefix.bech32):
        if addr[len(plbtc.config.current.prefix.bech32) + 1] == 'q':
            return script_pubkey_p2wpkh(addr)
        if addr[len(plbtc.config.current.prefix.bech32) + 1] == 'p':
            return script_pubkey_p2tr(addr)
    if plbtc.base58.decode(addr)[0] == plbtc.config.current.prefix.p2pkh:
        return script_pubkey_p2pkh(addr)
    if plbtc.base58.decode(addr)[0] == plbtc.config.current.prefix.p2sh:
...
def der_encode(r: plbtc.secp256k1.Fr, s: plbtc.secp256k1.Fr) -> bytearray:
...
def der_decode(sign: bytearray) -> typing.Tuple[plbtc.secp256k1.Fr, plbtc.secp256k1.Fr]:
    assert sign[0] == 0x30
    assert sign[1] == len(sign) - 2
    assert sign[2] == 0x02
    rlen = sign[3]
    r = plbtc.secp256k1.Fr(int.from_bytes(sign[4:4+rlen]))
    f = 4 + rlen
    assert sign[f] == 0x02
    slen = sign[f+1]
    f = f + 2
    s = plbtc.secp256k1.Fr(int.from_bytes(sign[f:f+slen]))
    return r, s
...
class Message:
    ...
    def hash(self) -> bytearray:
        ...
        b.extend(plbtc.core.compact_size_encode(24))
        b.extend(bytearray('Bitcoin Signed Message:\n'.encode()))
        b.extend(plbtc.core.compact_size_encode(len(self.data)))
        b.extend(bytearray(self.data.encode()))
        return plbtc.core.hash256(b)
    ...
    def pubkey(self, sig: str) -> PubKey:
        m = plbtc.secp256k1.Fr(int.from_bytes(self.hash()))
        sig = base64.b64decode(sig)
        assert sig[0] >= 27
        v = (sig[0] - 27) & 3
        r = plbtc.secp256k1.Fr(int.from_bytes(sig[0x01:0x21]))
        s = plbtc.secp256k1.Fr(int.from_bytes(sig[0x21:0x41]))
        return PubKey.pt_decode(plbtc.ecdsa.pubkey(m, r, s, v))
